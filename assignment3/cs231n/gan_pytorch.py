...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = (scores_real - 1).pow(2).mean() * 0.5 + scores_fake.pow(2).mean() * 0.5
...
def ls_generator_loss(scores_fake):
    ...
    loss = (scores_fake - 1).pow(2).mean() * 0.5
...
def build_dc_classifier(batch_size):
    ...
    model = nn.Sequential(
        ...
        nn.Conv2d(1, 32, (5, 5)),  # 24
        nn.LeakyReLU(),
        nn.MaxPool2d(2, 2),  # 12
        nn.Conv2d(32, 64, (5, 5)),  # 8
        nn.LeakyReLU(),
        nn.MaxPool2d(2, 2),  # 4
        Flatten(),  # 64*4*4
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    model = nn.Sequential(
        ...
        nn.ReLU(),
        nn.BatchNorm2d(64),
        nn.ConvTranspose2d(64, 1, (4, 4), stride=(2, 2), padding=(1, 1)),
        nn.Tanh(),
        Flatten()
    )
